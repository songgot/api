...
/**
 * @brief Enumeration for ml-offloading mode type.
 */
typedef enum
{
  ML_OFFLOADING_MODE_TYPE_DEFAULT = 0,
  ML_OFFLOADING_MODE_TYPE_TRAINING = 1,

  ML_OFFLOADING_TYPE_MAX
} ml_offloading_mode_type_e;
...
typedef struct
{
  ...
  ml_offloading_mode_type_e offloading_mode;
  void *priv;
...
_mlrs_process_service_offloading (nns_edge_data_h data_h, void *user_data)
{
  ...
  if (offloading_s->offloading_mode == ML_OFFLOADING_MODE_TYPE_TRAINING) {
...
/**
 * @brief Set ml service training offloading handle.
 */
void
    ...
    void *training_handle)
{
  ...
  offloading_s->offloading_mode = ML_OFFLOADING_MODE_TYPE_TRAINING;
  return;
}
