...
_sink_register_cb (ml_service_event_e event, ml_information_h event_data, void *user_data)
{
  ...
  if (output) {
    ...
    g_warning ("######## %s : %s: %d, name:%s >> [training_loss: %f, training_accuracy: %f, validation_loss: %f, validation_accuracy: %f]",
        __FILE__, __FUNCTION__, __LINE__, output_node_name, result_data[0],
...
TEST_F (MLServiceTrainingOffloading, registerPipeline)
{
  ...
  status = ml_service_set_event_cb (client_h, _sink_register_cb, NULL);
  EXPECT_EQ (status, ML_ERROR_NONE);
