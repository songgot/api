...
#define TRAINED_MODEL_FILE "@TRAINED_MODEL_FILE@"       /* combined with name set in conf */
...
typedef struct
{
  ...
  gchar *receiver_pipe_json_str;   /** @REMOTE_MODEL_CONFIG@, @REMOTE_MODEL@, @REMOTE_FILE_PATH@ in the receiver pipeline is converted to model_config_path, model_path, and data_path. */
  gchar *receiver_pipe;
  gchar *sender_pipe;
...
ml_service_training_offloading_stop (ml_service_s * mls)
{
  ...
  if (!training_s->pipeline_h)
    _ml_error_report_return (ret, "Pipeline is not constructed.");
